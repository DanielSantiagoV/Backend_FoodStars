...
export const esAdminMiddleware = (req, res, next) => {
    ...
    // req.usuario debería estar establecido por un middleware de autenticación anterior
    ...
    // Compara el rol del usuario con el rol ADMIN definido en las constantes
    // Si el rol no es ADMIN, el usuario no tiene permisos para acceder a esta ruta
    if (req.usuario.rol !== ROLES.ADMIN) {
        // Retorna un error 403 (Forbidden) indicando que el usuario no tiene permisos
        // Añade un mensaje específico explicando que se requiere rol de administrador
        return responderError(res, HTTP_STATUS.FORBIDDEN, ERROR_MESSAGES.FORBIDDEN + '. Se requiere rol de administrador.');
    }
    
    // Si todas las validaciones pasan, llama a next() para continuar con el siguiente middleware o controlador
